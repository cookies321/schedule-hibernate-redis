package admin.hnsd.commons.dao;
...
public class BaseDaoImpl<T> implements BaseDao<T> {
	...
	@Override
	public void insertBatch(List<T> list, int size) {
		...
		for (int i = 0 ; i < list.size() ; i++ ){
			...
			//每当累加器是batchSize的倍数时，将Session中的数据刷入数据库，并清空Session缓存
			if (i % size == 0){
				logger.info(list.get(i).getClass().getSimpleName()+"---------"+i);
				session.flush();
				session.clear();
				tx.commit();
